...
module SyntaxTree
  ...
  class AryPtn < Node
    ...
    def format(q)
      parts = [*requireds]
      parts << AryPtnRestFormatter.new(rest) if rest
      parts += posts
  ...
  class Params < Node
    ...
    def format(q)
      parts = [
        *requireds,
        *optionals.map { |(name, value)| ParamsOptionalFormatter.new(name, value) }
      ]
      ...
      parts +=
        [
          *posts,
          *keywords.map { |(name, value)| ParamsKeywordFormatter.new(name, value) }
        ]

      parts << ParamsKeywordRestFormatter.new(keyword_rest) if keyword_rest
