...
module ParseHelper
  ...
  CURRENT_VERSION = RUBY_VERSION.split(".")[0..1].join(".").freeze
  ...
  known_failures = [
    ...
    "test_pattern_matching_single_line_allowed_omission_of_parentheses:9581",
    "test_pattern_matching_single_line_allowed_omission_of_parentheses:9611",
  ...
  if CURRENT_VERSION <= "2.7"
    # I'm not sure why this is failing on 2.7.0, but we'll turn it off for now
    # until we have more time to investigate.
    todo_failures.push("test_pattern_matching_hash:*")
  end

  if CURRENT_VERSION <= "3.0"
    # In < 3.0, there are some changes to the way the parser gem handles
    # forwarded args. We should eventually support this, but for now we're going
    # to mark them as todo.
    todo_failures.push(
      "test_forward_arg:*",
  ...
  if CURRENT_VERSION == "3.1"
    # This test actually fails on 3.1.0, even though it's marked as being since
    # 3.1. So we're going to skip this test on 3.1, but leave it in for other
    # versions.
    known_failures.push(
  ...
  # This is the list of all failures.
  FAILURES = (known_failures + todo_failures).freeze
  ...
  def assert_parses(_ast, code, _source_maps = "", versions = ALL_VERSIONS)
    # We're going to skip any examples that aren't for the current version of
    # Ruby.
    return unless versions.include?(CURRENT_VERSION)

    # We're going to skip any examples that are for older Ruby versions that we
    # do not support.
    ...
    caller(1, 3).each do |line|
      ...
      return if FAILURES.include?("#{name}:#{lineno}")
      return if FAILURES.include?("#{name}:*")
    end
